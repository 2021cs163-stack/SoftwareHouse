...
export function demoAction(action,payload,requestId) {
 ...
 if(action==='create_project') {
  ...
  if(cents(deposit)<0 || cents(deposit)>cents(amount)) throw new Error('The deposit cannot exceed the contract amount.');
  data.projects.unshift({id,contract_id:payload.contract_id,name:payload.name,client:payload.client,type:payload.type,responsible:payload.responsible,start_date:payload.start_date,amount,status:'ongoing',completed_on:null,created_at:new Date().toISOString()});
  if(deposit>0) data.receipts.unshift({id:crypto.randomUUID(),project_id:id,amount:deposit,date:payload.deposit_date,note:'Initial deposit'});
 ...
 data.events.unshift({id:requestId,action,description:({create_project:'Project added',complete_project:'Project completed',receive_payment:'Payment received',add_expense:'Expense recorded',pay_partner:'Partner paid',renew_subscription:'Subscription renewed'}[action] || action)+' · '+(payload.reason || payload.contract_id || project?.contract_id || payload.partner || ''),amount:amount||null,created_at:new Date().toISOString(),actor:'Preview user'});
}
