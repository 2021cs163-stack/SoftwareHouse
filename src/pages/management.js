...
function projectRows(projects,data,showActions=true) {
 return projects.map(p=>{
  ...
  return '<tr class="project-row '+p.status+'"><td><strong>'+esc(p.name)+'</strong><small>'+esc(p.client)+'</small></td><td><span class="contract-code">'+esc(p.contract_id)+'</span></td><td>'+esc(p.type)+'</td><td><span class="person-mini">'+esc(initials(p.responsible))+'</span>'+esc(p.responsible)+'</td><td>'+dateLabel(p.start_date)+'</td><td>'+moneyCell(p.amount)+'</td><td>'+badge(p.status==='done'?'Done':'Ongoing',p.status==='done'?'green':'yellow')+'</td>'+(showActions?'<td><div class="row-actions"><button class="table-button" data-action="project-details" data-id="'+p.id+'">Details</button><button class="table-button" data-action="edit_project" data-id="'+p.id+'">Edit</button>'+(p.status==='ongoing'?'<button class="done-button" data-action="complete_project" data-id="'+p.id+'">'+icon('check')+'Done</button>':'<span class="date-small">'+dateLabel(p.completed_on)+'</span>')+'</div></td>':'')+'</tr>';
 });
}
...
export function managementPage(page,data,filters) {
 ...
 if(page==='dashboard') {
  return heading('THE BIG PICTURE','Business overview','Your projects, money, and partners. All in one place.','<div class="heading-actions">'+button('Add investment','add_investment','','secondary small')+button('New project','create_project')+'</div>')+
   '<div class="metrics-grid">'+metric('Available business cash',money(t.cash),'Receipts + investments − expenses − payouts','featured','receipts')+metric('Total received',money(t.received),data.receipts.length+' recorded payments','','receipts')+metric('Total expenses',money(t.expenses),data.expenses.length+' expense records','','expenses')+metric('Outstanding payments',money(t.remaining),'Contract amounts still to collect','','remaining')+'</div>'+
   ...
   '<div class="bottom-stats"><span>Partner investments <b>'+money(t.invested)+'</b></span><span>Net received after expenses <b>'+money(t.net)+'</b></span><span>Partner payouts <b>'+money(t.payouts)+'</b></span><span>Each partner’s 25% share <b>'+money(t.share)+'</b></span></div>';
 }
 if(page==='projects') {
  const projects=data.projects.filter(p=>matches(p,filters)&&(filters.status==='all'||p.status===filters.status));
  return heading('DELIVER GREAT WORK','Projects','Manage project details, client contacts, and actual payment balances.',button('New project','create_project'))+
 ...
 if(page==='receipts') {
  ...
  '<div class="metrics-grid three">'+metric('Total received',money(t.received),'Cash received from project clients','','receipts')+metric('Still to collect',money(t.remaining),'Across all project contracts','','remaining')+metric('Payment records',data.receipts.length,'Initial and subsequent payments','','receipts')+'</div>'+
 ...
 if(page==='remaining') {
  ...
  return heading('CLOSE THE LOOP','Remaining payments','The remaining amount you entered, reduced by subsequent client payments.',button('Record payment','receive_payment'))+
  ...
  panel('Outstanding contracts',toolbar(filters)+table(['Contract / client','Contract total','Received','Remaining','Project status',''],balances.map(p=>row(['<strong>'+esc(p.contract_id)+'</strong><small>'+esc(p.client)+'</small>',moneyCell(p.amount),moneyCell(p.received),'<strong class="text-yellow">'+money(p.remaining)+'</strong>',badge(p.status==='done'?'Done':'Ongoing',p.status==='done'?'green':'yellow'),'<button class="table-button" data-action="receive_payment" data-id="'+p.id+'">Record payment '+icon('arrow')+'</button>'])),'No outstanding balances.'))+
 ...
 if(page==='investments') {
  const investments=(data.investments||[]).filter(i=>matches(i,filters));
  return heading('FUND THE BUSINESS','Partner investments','Record capital contributed by each partner and see where your business funding comes from.',button('Add investment','add_investment'))+
   '<div class="metrics-grid three">'+metric('Total invested',money(t.invested),'All partner capital contributions','','investments')+
   metric('Available business cash',money(t.cash),'Includes capital and received payments','','receipts')+
   metric('Investment records',(data.investments||[]).length,'Every contribution is recorded','','investments')+'</div>'+
   '<div class="investment-partners">'+PARTNERS.map(name=>'<div><span class="person-mini">'+initials(name)+'</span><span>'+esc(name)+'<strong>'+money(partnerBalance(name,data).invested)+'</strong></span><button class="icon-button" data-action="add_investment" data-partner="'+esc(name)+'" aria-label="Add investment for '+esc(name)+'">'+icon('plus')+'</button></div>').join('')+'</div>'+
   '<div class="info-banner">Investments increase the money available to the business. They do not increase client income or the partners’ earned profit shares.</div>'+
   panel('Investment history',toolbar(filters)+table(['Partner','Amount invested','Date','Note'],investments.map(i=>row(['<strong>'+esc(i.partner)+'</strong>',moneyCell(i.amount),dateLabel(i.date),esc(i.note)||'—'])),'No investments recorded yet.'));
 }
 if(page==='partners') return heading('GROW TOGETHER','Partner payments','Four equal partners. A clear record of every distribution.','<div class="heading-actions">'+button('Add investment','add_investment','','secondary small')+button('Pay a partner','pay_partner')+'</div>')+
  '<div class="partner-grid">'+PARTNERS.map((name,i)=>{const b=partnerBalance(name,data);return '<article class="partner-card"><div class="partner-top"><span class="partner-avatar tone-'+i+'">'+initials(name)+'</span><span class="share-tag">25% SHARE</span></div><h2>'+name+'</h2><div class="partner-available"><small>Available to receive</small><strong class="'+(b.available<0?'text-red':'')+'">'+money(b.available)+'</strong></div><div class="partner-breakdown"><span>Capital invested <b>'+money(b.invested)+'</b></span><span>Earned share <b>'+money(b.share)+'</b></span><span>Already paid <b>'+money(b.paid)+'</b></span></div><button class="partner-pay" data-action="pay_partner" data-partner="'+name+'" '+(b.available<=0||t.cash<=0?'disabled':'')+'>Pay partner '+icon('arrow')+'</button></article>';}).join('')+'</div>'+
